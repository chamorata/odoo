...
class ResConfigSettings(models.TransientModel):
    ...
    manufacturing_lead = fields.Float(related='company_id.manufacturing_lead', string="Manufacturing Lead Time",
                                      readonly=False)
    use_manufacturing_lead = fields.Boolean(string="Default Manufacturing Lead Time",
    ...
    group_unlocked_by_default = fields.Boolean("Unlock Manufacturing Orders",
                                               implied_group='mrp.group_unlocked_by_default')
    group_mrp_reception_report = fields.Boolean("Allocation Report for Manufacturing Orders",
                                                implied_group='mrp.group_mrp_reception_report')
    group_mrp_workorder_dependencies = fields.Boolean("Work Order Dependencies",
    ...
    def set_values(self):
        ...
        if not self.group_mrp_workorder_dependencies:
            ...
            self.env['mrp.bom'].sudo().search(
                [('allow_operation_dependencies', '=', True)]).allow_operation_dependencies = False
    ...
    def _onchange_group_unlocked_by_default(self):
        ...
        if self.group_unlocked_by_default:
            self.env['mrp.production'].search(
                [('state', 'not in', ('cancel', 'done')), ('is_locked', '=', True)]).is_locked = False
        else:
            self.env['mrp.production'].search(
                [('state', 'not in', ('cancel', 'done')), ('is_locked', '=', False)]).is_locked = True
