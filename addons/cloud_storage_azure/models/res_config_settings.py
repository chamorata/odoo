...
class CloudStorageSettings(models.TransientModel):
    ...
    def _setup_cloud_storage_provider(self):
        ...
        upload_expiry = datetime.now(timezone.utc) + timedelta(
            seconds=self.env['ir.attachment']._cloud_storage_upload_url_time_to_expiry)
        upload_url = self.env['ir.attachment']._generate_cloud_storage_azure_sas_url(**blob_info, permission='c',
        ...
        download_expiry = datetime.now(timezone.utc) + timedelta(
            seconds=self.env['ir.attachment']._cloud_storage_download_url_time_to_expiry)
        download_url = self.env['ir.attachment']._generate_cloud_storage_azure_sas_url(**blob_info, permission='r',
    ...
    def _check_cloud_storage_uninstallable(self):
        ...
        if cr.fetchone():
            raise UserError(
                _('Some Azure attachments are in use, please migrate their cloud storages before disable this module'))
