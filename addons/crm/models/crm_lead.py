...
class Lead(models.Model):
    ...
    def get_empty_list_help(self, help_message):
        ...
        if alias_record and alias_record.alias_domain and alias_record.alias_name:
            sub_title = Markup(
                _('Use the <i>New</i> button, or send an email to %(email_link)s to test the email gateway.')) % {
                            'email_link': Markup("<b><a href='mailto:%s'>%s</a></b>") % (alias_record.alias_email,
                                                                                         alias_record.alias_email),
                        }
    ...
    def _merge_get_fields_specific(self):
        return {
            ...
            'priority': lambda fname, leads: max(priorities) if (
                priorities := leads.filtered('priority').mapped('priority')) else False,
        ...
        for opportunity_su in opportunities.sudo():
            for message_su in opportunity_su.message_ids:
                if message_su.subject:
                    subject = _("From %(source_name)s: %(source_subject)s", source_name=opportunity_su.name,
    ...
    def _get_customer_information(self):
        ...
        for record in self.filtered('email_normalized'):
            ...
            contact_name = record.contact_name or record.partner_name or parse_contact_from_email(record.email_from)[
    ...
    def _message_partner_info_from_emails(self, emails, link_mail=False):
        ...
        for email, partner_info in zip(emails, result):
            ...
            if email.lower() == self.email_from.lower() or (
                    email_normalized and self.email_normalized == email_normalized):
