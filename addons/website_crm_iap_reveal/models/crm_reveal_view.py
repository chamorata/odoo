...
class CRMRevealView(models.Model):
    ...
    reveal_state = fields.Selection([('to_process', 'To Process'), ('not_found', 'Not Found')], default='to_process',
    ...
    def _clean_reveal_views(self):
        ...
        weeks_valid = self.env['ir.config_parameter'].sudo().get_param('reveal.view_weeks_valid',
                                                                       DEFAULT_REVEAL_VIEW_WEEKS_VALID)
        try:
        ...
        domain.append(
            ('create_date', '<', fields.Datetime.to_string(datetime.date.today() - relativedelta(weeks=weeks_valid))))
