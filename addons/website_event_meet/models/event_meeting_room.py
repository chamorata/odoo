...
class EventMeetingRoom(models.Model):
    ...
    def _compute_website_url(self):
        ...
        for meeting_room in self:
            if meeting_room.id:
                ...
                meeting_room.website_url = '%s/event/%s/meeting_room/%s' % (base_url, self.env["ir.http"]._slug(
                    meeting_room.event_id), self.env["ir.http"]._slug(meeting_room))
    ...
    def open_website_url(self):
        ...
        return self.env['website'].get_client_action(
            f'/event/{self.env["ir.http"]._slug(self.event_id)}/meeting_room/{self.env["ir.http"]._slug(self)}')
