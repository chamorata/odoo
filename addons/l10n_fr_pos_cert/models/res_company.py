...
class ResCompany(models.Model):
    ...
    def _check_pos_hash_integrity(self):
        ...
        def build_order_info(order):
            ...
            return [ctx_tz(order, 'date_order'), order.l10n_fr_hash, order.name, order_reference_string,
        ...
        if self._is_accounting_unalterable():
            orders = self.env['pos.order'].search(
                [('state', 'in', ['paid', 'done', 'invoiced']), ('company_id', '=', self.id),
            ...
            raise UserError(
                _('Accounting is not unalterable for the company %s. This mechanism is designed for companies where accounting is unalterable.',
                  self.env.company.name))
