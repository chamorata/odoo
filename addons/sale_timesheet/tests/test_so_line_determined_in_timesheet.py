...
class TestSoLineDeterminedInTimesheet(TestCommonSaleTimesheet):
    ...
    def test_sol_determined_when_project_is_task_rate(self):
        ...
        self.assertTrue(all(sol.qty_delivered == 0 for sol in self.so.order_line),
        ...
        self.assertEqual(task.remaining_hours_so, 0,
    ...
    def test_sol_determined_when_project_is_project_rate(self):
        ...
        self.assertEqual(task.sale_line_id, self.so.order_line[0],
                         "The SOL in the task should be the one containing the prepaid service product.")
        self.assertTrue(all(sol.qty_delivered == 0 for sol in self.so.order_line),
                        "The quantity delivered should be equal to 0 because we have no timesheet for each SOL containing in the SO.")
        ...
        self.assertTrue(timesheet.so_line == task.sale_line_id == self.so.order_line[0],
                        "The SOL in the timesheet should be the same than the one in the task.")
        ...
        self.assertTrue(timesheet.so_line == task.sale_line_id == self.so.order_line[1],
                        "The SOL in the timesheet should also change and be the same than the one in the task.")
    ...
    def test_sol_determined_when_project_is_employee_rate(self):
        ...
        self.assertTrue(timesheet.so_line == task.sale_line_id == self.so.order_line[0],
                        "The SOL in the timesheet should be the same than the one in the task.")
        self.assertEqual(self.so.order_line[0].qty_delivered, 1,
        ...
        self.assertTrue(
            employee_user_timesheet.so_line == self.project_employee_rate.sale_line_employee_ids[0].sale_line_id ==
            self.so.order_line[1],
            "The SOL in the timesheet should be the one defined in the mapping for the employee user.")
        self.assertEqual(self.so.order_line[1].qty_delivered, 2,
                         "The quantity delivered for this SOL should be equal to 2 hours.")

        # 5) Change the SOL in the task and check if only the SOL in the timesheet which does not concerne about the mapping changes,
        task.update({'sale_line_id': self.so.order_line[2].id})
        self.assertTrue(timesheet.so_line == task.sale_line_id == self.so.order_line[2],
                        "The SOL in the timesheet should also change and be the same than the one in the task.")
        self.assertNotEqual(timesheet.so_line, employee_user_timesheet.so_line,
        ...
        self.assertTrue(employee_user_timesheet.so_line == mapping.sale_line_id == self.so.order_line[-1],
