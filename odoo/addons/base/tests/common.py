    ...
    def setUpClass(cls):
        ...
        if not cls.user_demo:
            ...
            cls.user_demo = cls.env['res.users'].create({
                ...
                'groups_id': [
                    Command.set([cls.env.ref('base.group_user').id, cls.env.ref('base.group_partner_manager').id])],
            })
...
class MockSmtplibCase:
    ...
    def mock_smtplib_connection(self):
        ...
        with patch('smtplib.SMTP_SSL', side_effect=lambda *args, **kwargs: self.testing_smtp_session), \
                ...
                patch.object(type(IrMailServer), '_find_mail_server',
                             mock_function(find_mail_server_origin)) as find_mail_server_mocked:
