...
class PosSession(models.Model):
    ...
    sequence_number = fields.Integer(string='Order Sequence Number',
    ...
    cash_journal_id = fields.Many2one('account.journal', compute='_compute_cash_journal', string='Cash Journal',
                                      store=True)
    ...
    statement_line_ids = fields.One2many('account.bank.statement.line', 'pos_session_id', string='Cash Lines',
                                         readonly=True)
    ...
    bank_payment_ids = fields.One2many('account.payment', 'pos_session_id', 'Bank Payments',
        ...
        if vals.get('state') == 'closed':
            for record in self:
                record.config_id._notify(
                    ('CLOSING_SESSION', {'login_number': self.env.context.get('login_number', False)}))
    ...
    def _load_pos_data_models(self, config_id):
        return ['pos.config', 'pos.order', 'pos.order.line', 'pos.pack.operation.lot', 'pos.payment',
                ...
                'product.attribute.custom.value', 'product.template.attribute.value', 'product.combo',
                ...
                'res.currency', 'pos.note', 'ir.ui.view', 'product.tag', 'ir.module.module']
        ...
        for session in self:
            ...
            if cash_payment_method:
                ...
                captured_cash_payments_domain = AND(
                    [session._get_captured_payments_domain(), [('payment_method_id', '=', cash_payment_method.id)]])
    ...
    def _compute_total_payments_amount(self):
        result = self.env['pos.payment']._read_group(self._get_captured_payments_domain(), ['session_id'],
                                                     ['amount:sum'])
    ...
    def _check_invoices_are_posted(self):
        unposted_invoices = self._get_closed_orders().sudo().with_company(self.company_id).account_move.filtered(
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if not vals.get('rescue'):
                ...
                sessions = self.sudo().search_read([('name', 'ilike', vals['name'])], ['name'], order='name desc',
                                                   limit=1)
                if len(sessions):
        ...
        if self.get_session_orders().filtered(lambda o: o.state != 'cancel') or self.sudo().statement_line_ids:
            ...
            if self.update_stock_at_closing:
                ...
                self._get_closed_orders().filtered(
                    lambda o: not o.is_total_cost_computed)._compute_total_cost_at_session_closing(
    ...
    def _get_diff_account_move_ref(self, payment_method):
        return _('Closing difference in %(payment_method)s (%(session)s)', payment_method=payment_method.name,
                 session=self.name)
    ...
    def _get_diff_vals(self, payment_method_id, diff_amount, outstanding_account=False):
        ...
        source_vals = self._debit_amounts({'account_id': source_account.id}, amounts['amount'],
                                          amounts['amount_converted'])
        dest_vals = self._credit_amounts({'account_id': destination_account.id}, amounts['amount'],
                                         amounts['amount_converted'])
    ...
    def get_closing_control_data(self):
        ...
        non_cash_payments_grouped_by_method_id = {pm: orders.payment_ids.filtered(lambda p: p.payment_method_id == pm)
    ...
    def _prepare_balancing_line_vals(self, imbalance_amount, move, balancing_account):
        ...
        if (not self.is_in_company_currency):
            imbalance_amount_session = self.company_id.currency_id._convert(imbalance_amount, self.currency_id,
                                                                            self.company_id,
    ...
    def _get_balancing_account(self):
        return (
                ...
                or self.env['res.partner']._fields['property_account_receivable_id'].get_company_dependent_fallback(
    ...
    def _accumulate_amounts(self, data):
        ...
        if self.company_id.anglo_saxon_accounting:
            all_picking_ids = self.order_ids.filtered(
                lambda p: not p.is_invoiced and not p.shipping_date).picking_ids.ids + self.picking_ids.filtered(
                lambda p: not p.pos_order_id).ids
            if all_picking_ids:
                # Combine stock lines
                ...
                for stock_moves_split in self.env.cr.split_for_in_conditions(stock_moves.ids):
                    ...
                    candidates = stock_moves_batch \
                        .filtered(lambda m: not bool(
                        m.origin_returned_move_id and sum(m.stock_valuation_layer_ids.mapped('quantity')) >= 0)) \
    ...
    def _create_bank_payment_moves(self, data):
        ...
        for bank_payment_method in self.payment_method_ids.filtered(
            ...
            self._create_diff_account_move_for_split_payment_method(bank_payment_method, bank_payment_method_diffs.get(
                bank_payment_method.id) or 0)
    ...
    def _create_combine_account_payment(self, payment_method, amounts, diff_amount):
        ...
        account_payment = self.env['account.payment'].with_context(pos_payment=True).create({
            ...
            'memo': _('Combine %(payment_method)s POS payments from %(session)s', payment_method=payment_method.name,
                      session=self.name),
        ...
        for stock_moves in [stock_output, stock_return]:
            for account, amounts in stock_moves.items():
                stock_output_vals[account].append(
                    self._get_stock_output_vals(account, amounts['amount'], amounts['amount_converted']))
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        if self.company_id.account_default_pos_receivable_account_id.reconcile:
            ...
            for payment in split_inv_payment_receivable_lines:
                lines = split_inv_payment_receivable_lines[payment] | split_invoice_receivable_lines.get(payment,
                                                                                                         self.env[
                                                                                                             'account.move.line'])
                lines.filtered(lambda line: not line.reconciled).with_context(no_cash_basis=True).reconcile()
    ...
    def _get_sale_vals(self, key, sale_vals):
        ...
        if applied_taxes:
            name = _('%(title)s %(product_name)s with %(taxes)s', title=title, product_name=product_name,
    ...
    def _get_other_related_moves(self):
        ...
        diff_lines_ref = [self._get_diff_account_move_ref(pm) for pm in self.payment_method_ids if
                          pm.type == 'bank' and pm.split_transactions]
    ...
    def set_opening_control(self, cashbox_value: int, notes: str):
        ...
        if not self.rescue:
            self.name = self.env['ir.sequence'].with_context(company_id=self.config_id.company_id.id).next_by_code(
                'pos.session')
    ...
    def _alert_old_session(self):
        ...
        sessions = self.sudo().search(
            [('start_at', '<=', (fields.datetime.now() - timedelta(days=7))), ('state', '!=', 'closed')])
        for session in sessions:
            if self.env['mail.activity'].search_count(
                    [('res_id', '=', session.id), ('res_model', '=', 'pos.session')]) == 0:
    ...
    def _get_pos_fallback_nomenclature_id(self):
        ...
        fallback_nomenclature_id = self.env['ir.config_parameter'].sudo().get_param(
            'point_of_sale.fallback_nomenclature_id')
    ...
    def _pos_has_valid_product(self):
        return self.env['product.product'].sudo().search_count(
            [('available_in_pos', '=', True), ('list_price', '>=', 0),
             ('id', 'not in', self.env['pos.config']._get_special_products().ids), '|', ('active', '=', False),
    ...
    def _update_session_info(self, session_info):
        ...
        session_info['user_companies'] = {'current_company': self.company_id.id, 'allowed_companies': {
            self.company_id.id: session_info['user_companies']['allowed_companies'][self.company_id.id]}}
