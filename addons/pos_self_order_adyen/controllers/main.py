...
class PosSelfAdyenController(PosAdyenController):
    ...
    def _process_payment_response(self, data, adyen_pm_sudo):
        ...
        if not order_sudo:
            _logger.warning('Received an Adyen event notification for the self order #%d that does not exist (anymore)',
                            self_order_id)
        ...
        order = order_sudo.sudo(False).with_user(
            order_sudo.session_id.config_id.self_ordering_default_user_id).with_company(
            order_sudo.session_id.config_id.company_id)
        ...
        if payment_result == 'Success' and order.config_id.self_ordering_mode == 'kiosk':
            payment_amount = data['SaleToPOIResponse']['PaymentResponse']['PaymentResult']['AmountsResp'][
                'AuthorizedAmount']
            card_type = \
            data['SaleToPOIResponse']['PaymentResponse']['PaymentResult']['PaymentInstrumentData']['CardData'][
                'PaymentBrand']
            transaction_id = data['SaleToPOIResponse']['PaymentResponse']['SaleData']['SaleTransactionID'][
                'TransactionID']
            order.add_payment({
        ...
        if order.config_id.self_ordering_mode == 'kiosk':
            order.config_id._notify('PAYMENT_STATUS', {
                ...
                'data': {
                    ...
                    'pos.order.line': order.lines.read(order._load_pos_self_data_fields(order.config_id.id),
                                                       load=False),
                }
            })
        return request.make_json_response(
