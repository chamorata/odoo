...
class TestBlogPerformance(UtilPerf):
    ...
    def test_20_perf_sql_blog_bigger_data_scaling(self):
        ...
        BlogPost.create(
            [{'name': 'Blog Post Test %s' % i, 'is_published': True, 'blog_id': blogs[i % 2].id} for i in range(1, 20)])
    ...
    def test_30_perf_sql_blog_bigger_data_scaling(self):
        ...
        BlogPost.create(
            [{'name': 'New Blog Post Test %s' % i, 'is_published': True, 'blog_id': blogs[random.randint(0, 1)].id} for
             i in range(1, 100)])
        blog_posts = BlogPost.search([])
        for blog_post in blog_posts:
