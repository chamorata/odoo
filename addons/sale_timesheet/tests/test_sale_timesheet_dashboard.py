...
class TestSaleTimesheetDashboard(Common):
    ...
    def test_get_sale_item_data_various_sol_with_timesheet_installed(self):
        ...
        self.assertEqual(
            ...
            [{'id': sol_service_3.id, 'name': 'Material', 'product_uom_qty': 1.0, 'qty_delivered': 0.0,
              'qty_invoiced': 0.0, 'product_uom': (1, 'Units'), 'product_id': (self.material_product.id, 'Material')}]
        )
        ...
        sale_item_data = self.dashboard_project.get_sale_items_data(with_action=False, limit=5,
                                                                    section_id='billable_milestones')
        self.assertEqual(
            sale_item_data['sol_items'],
        ...
        sale_item_data = self.dashboard_project.get_sale_items_data(with_action=False, limit=5,
                                                                    section_id='billable_time')
        self.assertEqual(
            sale_item_data['sol_items'],
            [{'id': sol_service_4.id, 'name': '[SERV-DELI1] Service delivered', 'product_uom_qty': 1.0,
              'qty_delivered': 0.0, 'qty_invoiced': 0.0, 'product_uom': (4, 'Hours'),
              'product_id': (self.dashboard_product_delivery_timesheet.id, '[SERV-DELI1] Service delivered')}]
        )
        sale_item_data = self.dashboard_project.get_sale_items_data(with_action=False, limit=5,
                                                                    section_id='billable_manual')
        self.assertEqual(
            sale_item_data['sol_items'],
            [{'id': sol_service_5.id, 'name': '[SERV-ORDERED2] Service Delivery', 'product_uom_qty': 1.0,
              'qty_delivered': 0.0, 'qty_invoiced': 0.0, 'product_uom': (4, 'Hours'),
