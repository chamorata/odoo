...
class TestSaleService(TestCommonSaleTimesheet):
    ...
    def test_sale_service(self):
        ...
        task = project.task_ids.filtered(
            lambda t: t.name == '%s - %s' % (self.sale_order.name, self.product_delivery_timesheet2.name))
        self.assertTrue(task, 'Sale Service: task is not created, or it badly named')
        self.assertEqual(task.partner_id, self.sale_order.partner_id,
        ...
        self.assertTrue(
            sale_order_line.product_uom_qty == sale_order_line.qty_delivered == sale_order_line.qty_invoiced,
        ...
        self.assertEqual(self.sale_order.tasks_count, 2,
        ...
        task.unlink()
        self.assertFalse(sale_order_line.task_id,
    ...
    def test_task_so_line_assignation(self):
        ...
        self.assertTrue(
            all([billing_type == 'billable_time' for billing_type in timesheets.mapped('timesheet_invoice_type')]),
            "All timesheets linked to the task should be on 'billable time'")
        self.assertEqual(so_line_deliver_global_project.qty_to_invoice, 5,
        ...
        self.assertEqual(task_serv2.timesheet_ids.mapped('so_line'), so_line_deliver_global_project,
    ...
    def test_delivered_quantity(self):
        ...
        self.assertEqual(so_line_deliver_new_task_project.qty_delivered, timesheet1.unit_amount,
                         'Delivered quantity should be the same then its only related timesheet.')

        # remove the only timesheet
        timesheet1.unlink()
        self.assertEqual(so_line_deliver_new_task_project.qty_delivered, 0.0,
        ...
        self.assertEqual(so_line_deliver_new_task_project.qty_delivered,
                         timesheet2.unit_amount + timesheet3.unit_amount,
                         'Delivered quantity should be the sum of the 2 timesheets unit amounts.')

        # remove timesheet2
        timesheet2.unlink()
        self.assertEqual(so_line_deliver_new_task_project.qty_delivered, timesheet3.unit_amount,
    ...
    def test_sale_create_task(self):
        ...
        self.assertEqual(so_line1.task_id.sale_line_id, so_line1,
        ...
        self.assertEqual(so_line1.product_uom_qty, so_line1.project_id.allocated_hours,
                         "The planned hours on the project should be the same as the ordered quantity of the native SO line")
        self.assertEqual(so_line1.product_uom_qty, so_line1.task_id.allocated_hours,
    ...
    def test_sale_create_project(self):
        ...
        self.assertTrue(so_line1.project_id in self.project_template_state.project_ids,
                        "Stage 1 from template B should be part of project from so line 1")
        self.assertTrue(so_line1.project_id in self.project_template_state.project_ids,
                        "Stage 1 from template B should be part of project from so line 1")

        self.assertTrue(so_line5.project_id in stage1_tmpl2.project_ids,
                        "Stage 1 from template B should be part of project from so line 5")
        self.assertTrue(so_line5.project_id in stage2_tmpl2.project_ids,
        ...
        self.assertEqual(so_line4.task_id.project_id, so_line2.project_id,
                         "Task created with line 4 should have the project based on template A of the SO.")

        self.assertEqual(so_line1.project_id.sale_line_id, so_line1,
                         "SO line of project with template A should be the one that create it.")
        self.assertEqual(so_line2.project_id.sale_line_id, so_line2,
                         "SO line of project should be the one that create it.")
        self.assertEqual(so_line5.project_id.sale_line_id, so_line5,
    ...
    def test_billable_task_and_subtask(self):
        ...
        self.assertEqual(project.sale_line_id, so_line_deliver_new_task_project,
                         "The created project should be linked to the so line")
        self.assertEqual(task.sale_line_id, so_line_deliver_new_task_project,
        ...
        self.assertEqual(task2.sale_line_id, project.sale_line_id,
                         "A new task in a billable project should have the same SO line as its project")
        self.assertEqual(task2.partner_id, so_line_deliver_new_task_project.order_partner_id,
                         "A new task in a billable project should have the same SO line as its project")
        ...
        self.assertEqual(task.sale_line_id, so_line_deliver_new_task_project_2,
        ...
        self.assertEqual(subtask.sale_line_id, so_line_deliver_new_task_project_2,
    ...
    def test_change_ordered_qty(self):
        ...
        self.sale_order.action_confirm()
        self.assertEqual(sale_order_line.product_uom_qty, sale_order_line.task_id.allocated_hours,
    ...
    def test_copy_billable_project_and_task(self):
        ...
        self.assertEqual(len(project.tasks), len(project_copy.tasks),
                         "Copied project must have the same number of tasks")
        self.assertFalse(project_copy.tasks.mapped('sale_line_id'),
    ...
    def test_remaining_hours_prepaid_services(self):
        ...
        self.assertEqual(len(prepaid_service_sol), 1,
                         "It should only have one SOL with prepaid service product in this SO.")
        self.assertEqual(prepaid_service_sol.remaining_hours,
        ...
        self.assertEqual(task.remaining_hours_so, 1,
        ...
        self.assertEqual(prepaid_service_sol.remaining_hours, task.remaining_hours_so,
        ...
        self.assertEqual(timesheet.so_line, prepaid_service_sol,
    ...
    def test_prevent_update_project_allocated_hours_after_confirming_quotation(self):
        ...
        self.assertEqual(allocated_hours, order_line.project_id.allocated_hours,
    ...
    def test_compute_show_timesheet_button(self):
        ...
        sale_order_1.action_confirm()
        self.assertFalse(sale_order_1.show_hours_recorded_button,
        ...
        self.assertTrue(sale_order_1.show_hours_recorded_button,
                        "There is a product service with the service_policy set on 'delivered on timesheet' and a project on the sale order, the button should be displayed")
        ...
        self.assertFalse(sale_order_2.show_hours_recorded_button,
                         "There is a product service with the service_policy set on 'delivered on timesheet' but no project on the sale order, the button should be hidden")
        line_1.project_id = self.project_global
        sale_order_2._compute_show_hours_recorded_button()
        self.assertTrue(sale_order_2.show_hours_recorded_button,
                        "There is a product service with the service_policy set on 'delivered on timesheet' and a project on the sale order, the button should be displayed")
    ...
    def test_service_product_uom_default(self):
        ...
        product = product_form.save()
        self.assertEqual(product.uom_id, uom_hour,
                         "UoM should be hours for timesheet service when default is not a time unit")
