...
class StockLocation(models.Model):
        ...
        for location in self:
            ...
            if location.is_subcontracting_location and location.usage != 'internal':
                raise ValidationError(
                    _("In order to manage stock accurately, subcontracting locations must be type Internal, linked to the appropriate company."))
            ...
            if reference_location_id:
                ...
                for location in locations:
                    existing_rules = {
                        ...
                        for rule in self.env['stock.rule'].with_context(active_test=False).search(
    ...
    def _archive_subcontracting_location_rules(self):
        ...
        reference_rules = self.env['stock.rule'].search(['|', ('location_src_id', 'in', reference_location_ids.ids),
        ...
        rules_to_archive = self.env['stock.rule'].search(
            ['&', ('route_id', 'in', reference_routes.ids), '|', ('location_src_id', 'in', self.ids),
