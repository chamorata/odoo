...
class SupplierInfo(models.Model):
    ...
    purchase_requisition_id = fields.Many2one('purchase.requisition',
                                              related='purchase_requisition_line_id.requisition_id', string='Agreement')
...
class ProductProduct(models.Model):
    ...
    def _prepare_sellers(self, params=False):
        ...
        if params and params.get('order_id') and params['order_id']._fields.get("requisition_id"):
            return sellers.filtered(lambda s: not s.purchase_requisition_id or s.purchase_requisition_id == params[
