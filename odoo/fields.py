...
class Field(MetaField('DummyField', (object,), {}), typing.Generic[T]):
    ...
    def _get_attrs(self, model_class, name):
        ...
        if 'group_operator' in attrs:
            warnings.warn("Since Odoo 18, 'group_operator' is deprecated, use 'aggregator' instead", DeprecationWarning,
                          2)
    ...
    def setup_related(self, model):
        ...
        if self.store and self.translate:
            _logger.warning("Translated stored related field (%s) will not be computed correctly in all languages",
                            self)
...
class Float(Field[float]):
    ...
    def __init__(self, string: str | Sentinel = SENTINEL, digits: str | tuple[int, int] | None | Sentinel = SENTINEL,
                 **kwargs):
...
class _String(Field[str | typing.Literal[False]]):
    ...
    def convert_to_record(self, value, record):
        ...
        if callable(self.translate) and record.env.context.get('edit_translations'):
            ...
            if lang != base_lang:
                base_value = record.with_context(edit_translations=None, check_translations=True, lang=base_lang)[
                    self.name]
            ...
            delay_translation = value != \
                                record.with_context(edit_translations=None, check_translations=None, lang=lang)[
                                    self.name]
                ...
                if old_term not in new_terms:
                    ...
                    if matches:
                        ...
                        if old_is_text or not closest_is_text:
                            if not closest_is_text and records.env.context.get(
                                    "install_mode") and lang == 'en_US' and term_adapter:
                                adapter = term_adapter(closest_term)
                                if adapter(old_term) is None:  # old term and closest_term have different structures
                                    continue
                                translation_dictionary[closest_term] = {k: adapter(v) for k, v in
                                                                        translation_dictionary.pop(old_term).items()}
                            else:
...
class Binary(Field):
    ...
    def convert_to_column(self, value, record, values=None, validate=True):
        ...
        try:
            ...
            raise UserError(
                record.env._("ASCII characters are required for %(value)s in %(field)s", value=value, field=self.name))
...
class _Relational(Field[M], typing.Generic[M]):
    ...
    def _description_domain(self, env):
        domain = self.domain(env[self.model_name]) if callable(
            self.domain) else self.domain  # pylint: disable=not-callable
...
class _RelationalMulti(_Relational[M], typing.Generic[M]):
    ...
    def write_batch(self, records_commands_list, create=False):
        ...
        if all(record_ids):
            ...
            assert not any(
                record_ids), f"{records_commands_list} contains a mix of real and new records. It is not supported."
