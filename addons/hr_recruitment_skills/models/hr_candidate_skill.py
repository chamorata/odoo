        ...
        for candidate_skill in self:
            if candidate_skill.skill_id not in candidate_skill.skill_type_id.skill_ids:
                raise ValidationError(
                    _("The skill %(name)s and skill type %(type)s doesn't match", name=candidate_skill.skill_id.name,
                      type=candidate_skill.skill_type_id.name))
        ...
        for candidate_skill in self:
            if candidate_skill.skill_level_id not in candidate_skill.skill_type_id.skill_level_ids:
                raise ValidationError(_("The skill level %(level)s is not valid for skill type: %(type)s",
                                        level=candidate_skill.skill_level_id.name,
                                        type=candidate_skill.skill_type_id.name))
    ...
    def _compute_skill_level_id(self):
        for candidate_skill in self:
            if not candidate_skill.skill_id:
                ...
                candidate_skill.skill_level_id = skill_levels.filtered('default_level') or skill_levels[
