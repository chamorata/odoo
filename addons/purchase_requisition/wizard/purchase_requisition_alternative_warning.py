...
class PurchaseRequisitionAlternativeWarning(models.TransientModel):
    ...
    alternative_po_ids = fields.Many2many('purchase.order', 'warning_purchase_order_alternative_rel',
    ...
    def action_cancel_alternatives(self):
        ...
        self.alternative_po_ids.filtered(
            lambda po: po.state in ['draft', 'sent', 'to approve'] and po.id not in self.po_ids.ids).button_cancel()
        return self._action_done()
