...
class EfakturDocument(models.Model):
    ...
    def _generate_xml(self, regenerate=False):
        ...
        if no_trx_code_entries:
            raise UserError(
                _("Some documents don't have a transaction code: %s", ", ".join(no_trx_code_entries.mapped('name'))))
        if non_invoice_entries:
            raise UserError(
                _("Some documents are not Customer Invoices: %s", ", ".join(non_invoice_entries.mapped('name'))))
    ...
    def _generate_efaktur_invoice(self):
        ...
        xml_content = self.env['ir.qweb']._render('l10n_id_efaktur_coretax.efaktur_coretax_template',
                                                  {'data': invoice_data, 'TIN': self.company_id.vat})
        return etree.tostring(cleanup_xml_node(xml_content, remove_blank_text=False, remove_blank_nodes=False),
