...
class ProjectStageDelete(models.TransientModel):
    ...
    stage_ids = fields.Many2many('project.project.stage', string='Stages To Delete', ondelete='cascade',
                                 context={'active_test': False}, export_string_translation=False)
    ...
    def _compute_projects_count(self):
        for wizard in self:
            wizard.projects_count = self.with_context(active_test=False).env['project.project'].search_count(
    ...
    def action_archive(self):
        projects = self.with_context(active_test=False).env['project.project'].search(
