...
class ResConfigSettings(models.TransientModel):
    ...
    lead_mining_in_pipeline = fields.Boolean("Create a lead mining request directly from the opportunity pipeline.",
                                             config_parameter='crm.lead_mining_in_pipeline')
    predictive_lead_scoring_start_date = fields.Date(string='Lead Scoring Starting Date',
    ...
    predictive_lead_scoring_fields = fields.Many2many('crm.lead.scoring.frequency.field',
    ...
    def _onchange_crm_auto_assignment_run_datetime(self):
        if self.crm_auto_assignment_interval_number <= 0:
            ...
            raise exceptions.UserError(
                _('Invalid repeat frequency. Consider changing frequency type instead of using large numbers.'))
    ...
    def _compute_pls_fields(self):
        ...
        for setting in self:
            if setting.predictive_lead_scoring_fields_str:
                ...
                setting.predictive_lead_scoring_fields = self.env['crm.lead.scoring.frequency.field'].search(
