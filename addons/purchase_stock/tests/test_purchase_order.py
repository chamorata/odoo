...
class TestPurchaseOrder(ValuationReconciliationTestCommon):
    ...
    def test_00_purchase_order_flow(self):
        ...
        if self.partner_a in self.product_id_2.seller_ids.partner_id:
            id_to_remove = self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a).ids[
                0] if self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a) else False
            if id_to_remove:
                self.product_id_2.write({
                    'seller_ids': [(2, id_to_remove, False)],
                })
        self.assertFalse(self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a),
        ...
        self.assertTrue(self.product_id_2.seller_ids.filtered(lambda r: r.partner_id == self.partner_a),
                        'Purchase: the partner should be in the list of the product suppliers')

        seller = self.product_id_2._select_seller(partner_id=self.partner_a, quantity=2.0, date=self.po.date_planned,
        ...
        if price_unit and seller and self.po.currency_id and seller.currency_id != self.po.currency_id:
            price_unit = seller.currency_id._convert(price_unit, self.po.currency_id, self.po.company_id,
                                                     self.po.date_order)
    ...
    def test_02_po_return(self):
        ...
        self.assertEqual(received_qty, 10.0,
                         'Purchase: Received quantity should be 10.0 instead of %s after validating incoming shipment' % received_qty)
        ...
        self.assertEqual(self.po.order_line[0].qty_received, 3.0,
    ...
    def test_04_update_date_planned(self):
        ...
        self.assertEqual(
            ...
            '<p>Those dates have been updated accordingly on the receipt %s.</p>' % (today.date(), tomorrow.date(),
                                                                                     po.picking_ids.name),
            activity.note,
        )
    ...
    def test_04_multi_uom(self):
        ...
        picking.move_ids.picked = True
        picking.button_validate()
        self.assertEqual(po.order_line.mapped('qty_received'), [4.0],
    ...
    def test_message_qty_already_received(self):
        ...
        self.assertIsNotNone(re.search(r"Received Quantity: 5.0 -&gt; 10.0", _message_content),
    ...
    def test_receive_returned_product_without_po_update(self):
        ...
        wizard = Form(self.env['stock.return.picking'].with_context(active_ids=receipt01.ids, active_id=receipt01.id,
        ...
        wizard = Form(
            self.env['stock.return.picking'].with_context(active_ids=return_pick.ids, active_id=return_pick.id,
                                                          active_model='stock.picking')).save()
        wizard.product_return_moves.quantity = 5
        wizard.product_return_moves.to_refund = False
        res = wizard.action_create_returns()
