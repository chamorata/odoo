...
class FleetVehicleModelCategory(models.Model):
    ...
    weight_capacity_uom_name = fields.Char(string='Weight unit of measure label',
                                           compute='_compute_weight_capacity_uom_name')
    volume_capacity = fields.Float(string="Max Volume")
    volume_capacity_uom_name = fields.Char(string='Volume unit of measure label',
    ...
    def _compute_display_name(self):
        ...
        for record in self:
            ...
            if record.weight_capacity:
                additional_info.append(_("%(weight_capacity)s %(weight_uom)s", weight_capacity=record.weight_capacity,
                                         weight_uom=record.weight_capacity_uom_name))
            if record.volume_capacity:
                additional_info.append(_("%(volume_capacity)s %(volume_uom)s", volume_capacity=record.volume_capacity,
                                         volume_uom=record.volume_capacity_uom_name))
            if additional_info:
                additional_info = format_list(self.env, additional_info, "unit-short")
                record.display_name = _("%(display_name)s (%(load_capacity)s)", display_name=record.display_name,
