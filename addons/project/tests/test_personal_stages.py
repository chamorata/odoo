...
class TestPersonalStages(TestProjectCommon):
    ...
    def test_personal_stage_base(self):
        ...
        self.assertFalse(
            self.env['project.task'].browse(self.task_1.id).with_user(self.user_projectmanager).personal_stage_type_id,
    ...
    def test_personal_stage_search(self):
        ...
        tasks = self.env['project.task'].with_user(self.user_projectuser).search(
            [('personal_stage_type_id', '=', self.user_stages[0].id)])
    ...
    def test_personal_stage_read_group(self):
        ...
        read_group_manager = self.env['project.task'].with_user(self.user_projectmanager).read_group(
            [('user_ids', '=', self.user_projectmanager.id)], fields=['sequence:avg'],
    ...
    def test_default_personal_stage(self):
        ...
        self.assertEqual(len(stages), 7,
                         "As this user had no personal stage, the default ones should have been created for him")
    ...
    def test_delete_personal_stage(self):
        ...
        self.assertEqual(self.env['project.task.type'].with_user(user_1.id).search_count(
            [('project_ids', '=', False), ('user_id', '=', user_1.id)]), 5)
        self.assertEqual(self.env['project.task'].with_user(user_1.id).search_count([('user_ids', 'in', user_1.ids)]),
                         4)
        ...
        self.assertEqual(self.env['project.task.type'].with_user(user_2.id).search_count(
            [('project_ids', '=', False), ('user_id', '=', user_2.id)]), 3)
        self.assertEqual(self.env['project.task'].with_user(user_2.id).search_count([('user_ids', 'in', user_2.ids)]),
                         2)
        private_tasks.invalidate_recordset(['personal_stage_type_id'])
        ...
        self.assertEqual(self.env['project.task.type'].with_user(user_2.id).search_count(
            [('project_ids', '=', False), ('user_id', '=', user_2.id)]), 2,
        ...
        self.assertEqual(self.env['project.task.type'].with_user(user_1.id).search_count(
            [('project_ids', '=', False), ('user_id', '=', user_1.id)]), 4,
        ...
        user_1_stages.filtered(lambda s: s.id in [user_1_stages[1].id, user_1_stages[3].id]).with_user(
            user_1.id).unlink()
        self.assertEqual(self.env['project.task.type'].with_user(user_1.id).search_count(
            [('project_ids', '=', False), ('user_id', '=', user_1.id)]), 2,
        ...
        self.assertEqual(self.env['project.task.type'].with_user(user_1.id).search_count(
            [('project_ids', '=', False), ('user_id', '=', user_1.id)]), 1,
                         "Superuser should be able to delete personal stages in batch.")
        self.assertEqual(self.env['project.task.type'].with_user(user_2.id).search_count(
            [('project_ids', '=', False), ('user_id', '=', user_2.id)]), 1,
                         "Superuser should be able to delete personal stages in batch.")
        self.assertEqual(self.env['project.task'].with_user(user_1.id).search_count([('user_ids', 'in', user_1.ids)]),
                         4,
