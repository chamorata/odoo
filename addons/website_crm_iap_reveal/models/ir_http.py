            ...
            if not (visitor_sudo and visitor_sudo.lead_ids):
                ...
                if country_code:
                    try:
                        ...
                        before = time.time()
                        new_rules_excluded = request.env['crm.reveal.view'].sudo()._create_reveal_view(website_id, url,
                                                                                                       ip_address,
                                                                                                       country_code,
                                                                                                       state_code,
                                                                                                       rules_excluded)
                        ...
                        if new_rules_excluded:
                            response.set_cookie('rule_ids', ','.join(new_rules_excluded), expires=None,
