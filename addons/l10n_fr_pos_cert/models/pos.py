...
ORDER_FIELDS_BEFORE_17_4 = ['date_order', 'user_id', 'lines', 'payment_ids', 'pricelist_id', 'session_id',
                            'pos_reference', 'sale_journal', 'fiscal_position_id', 'partner_id']
ORDER_FIELDS_FROM_17_4 = ['date_order', 'user_id', 'lines', 'payment_ids', 'pricelist_id', 'session_id',
                          'pos_reference', 'sale_journal', 'fiscal_position_id', 'pos_version']
...
class pos_order(models.Model):
    ...
    previous_order_id = fields.Many2one('pos.order', string='Previous Order', readonly=True,
                                        compute='_compute_previous_order', store=True, copy=False)
        ...
        for order in self:
            if order.company_id._is_accounting_unalterable():
                ...
                if (order.state in ['paid', 'done', 'invoiced'] and set(vals).intersection(ORDER_FIELDS)):
                    raise UserError(
                        _('According to the French law, you cannot modify a point of sale order. Forbidden fields: %s.') % ', '.join(
                            ORDER_FIELDS))
                # restrict the operation in case we are trying to overwrite existing hash
                if (order.l10n_fr_hash and 'l10n_fr_hash' in vals) or (
                        order.l10n_fr_secure_sequence_number and 'l10n_fr_secure_sequence_number' in vals):
                    raise UserError(
...
class PosOrderLine(models.Model):
    ...
    def write(self, vals):
        ...
        if set(vals).intersection(LINE_FIELDS):
            if any(l.company_id._is_accounting_unalterable() and l.order_id.state in ['done', 'invoiced'] for l in
                   self):
                raise UserError(
                    _('According to the French law, you cannot modify a point of sale order line. Forbidden fields: %s.') % ', '.join(
                        LINE_FIELDS))
