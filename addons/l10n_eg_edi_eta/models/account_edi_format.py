...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_eg_eta_connect_to_server(self, request_data, request_url, method, is_access_token_req=False,
        ...
        api_domain = is_access_token_req and self._l10n_eg_get_eta_token_domain(
            production_enviroment) or self._l10n_eg_get_eta_api_domain(production_enviroment)
        request_url = api_domain + request_url
        try:
            session = requests.session()
            session.mount("https://", LegacyHTTPAdapter())
            request_response = session.request(method, request_url, data=request_data.get('body'),
                                               headers=request_data.get('header'), timeout=(5, 10))
    ...
    def _cancel_invoice_edi_eta(self, invoice):
        ...
        if invoice.l10n_eg_submission_number:
            ...
            if document_summary.get('doc_data') and document_summary['doc_data'][0].get('status') in ('Cancelled',
                                                                                                      'Rejected'):
                return {'success': True}
        ...
        response_data = self._l10n_eg_eta_connect_to_server(request_data, request_url, 'PUT',
                                                            production_enviroment=invoice.company_id.l10n_eg_production_env)
        if response_data.get('error'):
            return response_data
    ...
    def _l10n_eg_get_einvoice_document_summary(self, invoice):
        ...
        response_data = self._l10n_eg_eta_connect_to_server(request_data, request_url, 'GET',
                                                            production_enviroment=invoice.company_id.l10n_eg_production_env)
        if response_data.get('error'):
            return response_data
        response_data = response_data.get('response').json()
        document_summary = [doc for doc in response_data.get('documentSummary', []) if
                            doc.get('uuid') == invoice.l10n_eg_uuid]
    ...
    def _l10n_eg_eta_get_access_token(self, invoice):
        ...
        request_data = {'body': {'grant_type': 'client_credentials'},
    ...
    def _l10n_eg_get_eta_invoice_pdf(self, invoice):
        ...
        request_data = {'body': None, 'header': {'Content-Type': 'application/json',
                                                 'Authorization': 'Bearer %s' % access_data.get('access_token')}}
        response_data = self._l10n_eg_eta_connect_to_server(request_data, request_url, 'GET',
                                                            production_enviroment=invoice.company_id.l10n_eg_production_env)
        if response_data.get('error'):
    ...
    def _l10n_eg_eta_prepare_invoice_lines_data(self, invoice, base_lines_aggregated_values):
        ...
        for base_line, aggregated_values in base_lines_aggregated_values:
            ...
            price_subtotal_before_discount = self._l10n_eg_edi_round(
                abs(line.balance / (1 - (line.discount / 100)))) if line.discount != 100.0 else self._l10n_eg_edi_round(
