...
class CalendarProviderConfig(models.TransientModel):
    ...
    cal_sync_paused = fields.Boolean(
        ...
        default=lambda self: str2bool(self.env['ir.config_parameter'].get_param('google_calendar_sync_paused'),
                                      default=False))
    ...
    microsoft_outlook_sync_paused = fields.Boolean(
        ...
        default=lambda self: str2bool(self.env['ir.config_parameter'].get_param('microsoft_calendar_sync_paused'),
                                      default=False))
    ...
    def action_calendar_prepare_external_provider_sync(self):
        ...
        if self.external_calendar_provider == 'google':
            ...
            self.env['ir.config_parameter'].set_param('microsoft_calendar_client_id',
                                                      self.microsoft_outlook_client_identifier)
            self.env['ir.config_parameter'].set_param('microsoft_calendar_client_secret',
                                                      self.microsoft_outlook_client_secret)
            self.env['ir.config_parameter'].set_param('microsoft_calendar_sync_paused',
                                                      self.microsoft_outlook_sync_paused)
