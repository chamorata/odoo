...
class TestProjectSharingCommon(TestProjectCommon):
    ...
    def setUpClass(cls):
        ...
        project_sharing_stages_vals_list = [
            ...
            (0, 0, {'name': 'Done', 'sequence': 10, 'fold': True,
                    'rating_template_id': cls.env.ref('project.rating_project_request_email_template').id}),
        ]
...
class TestProjectSharing(TestProjectSharingCommon):
    ...
    def test_project_share_wizard(self):
        ...
        project_share_form = Form(self.env['project.share.wizard'].with_context(active_model='project.project',
        ...
        self.assertEqual(len(self.project_portal.collaborator_ids), 1,
        ...
        self.assertTrue(self.project_portal.with_user(self.user_portal)._check_project_sharing_access(),
                        'The portal user should have read access to the portal project with project sharing feature.')
        project_share_wizard = self.env['project.share.wizard'].with_context(active_model="project.project",
                                                                             active_id=self.project_portal.id).new({})
        self.assertEqual(len(project_share_wizard.collaborator_ids), 1,
                         'The access right added in project share wizard should be added in the project when the user confirm the access in the wizard.')
        self.assertDictEqual({
    ...
    def test_project_share_wizard_add_collaborator_with_limited_access(self):
        ProjectShare = self.env['project.share.wizard'].with_context(active_model="project.project",
                                                                     active_id=self.project_portal.id)
        ...
        project_share_wizard = project_share_form.save()
        project_share_wizard.action_send_mail()
        self.assertEqual(len(self.project_portal.collaborator_ids), 2,
                         'The access right added in project share wizard should be added in the project when the user confirm the access in the wizard.')
        ...
        self.assertEqual(len(project_share_wizard.collaborator_ids), 2,
                         'The access right added in project share wizard should be added in the project when the user confirm the access in the wizard.')
    ...
    def test_project_share_wizard_remove_collaborators(self):
        PortalShare = self.env['project.share.wizard'].with_context(active_model="project.project",
                                                                    active_id=self.project_portal.id)
        self.project_portal.write({
            'collaborator_ids': [
        ...
        with Form(PortalShare) as project_share_form:
            self.assertEqual(len(project_share_form.collaborator_ids), 3,
            ...
            collaborator_ids_to_remove = {c_id for c_id, vals in collaborator_vals_per_id.items() if
                                          vals['access_mode'] != 'read'}
            index = 0
        ...
        self.assertIn(self.partner_2, self.project_portal.message_partner_ids,
                      "The readonly partner should still be a follower.")
        self.assertNotIn(self.user_portal.partner_id, self.project_portal.message_partner_ids,
                         "The readonly partner should still be a follower.")
        self.assertNotIn(self.partner_1, self.project_portal.message_partner_ids,
                         "The readonly partner should still be a follower.")
    ...
    def test_project_share_wizard_alter_access_mode_collaborators(self):
        ...
        self.assertEqual(len(self.project_portal.collaborator_ids), 3,
    ...
    def test_project_sharing_access(self):
        ...
        self.assertTrue(self.project_portal.with_user(self.user_projectuser)._check_project_sharing_access(),
                        'The internal user should have all accesses to project sharing feature of the portal project.')
        self.assertFalse(self.project_portal.with_user(self.user_portal)._check_project_sharing_access(),
    ...
    def test_create_task_in_project_sharing(self):
        ...
        with self.get_project_sharing_form_view(Task, self.user_portal) as form:
            ...
            self.assertFalse(task.child_ids.portal_user_names,
        ...
        with self.assertRaisesRegex(AccessError, "top-secret records"):
            Task.with_context(default_child_ids=[Command.update(self.task_no_collabo.id, {'name': 'Foo'})]).create(
                {'name': 'foo'})
        with self.assertRaisesRegex(AccessError, "top-secret records"):
        ...
        with self.assertRaisesRegex(AccessError, "not allowed to modify 'Project Tags'"):
            Task.with_context(default_tag_ids=[Command.update(self.task_tag.id, {'name': 'Bar'})]).create(
                {'name': 'foo'})
        with self.assertRaisesRegex(AccessError, "not allowed to delete 'Project Tags'"):
    ...
    def test_edit_task_in_project_sharing(self):
        ...
        with self.assertRaises(AccessError,
            ...
            with self.get_project_sharing_form_view(
                    self.task_cow.with_context({'tracking_disable': True, 'default_project_id': self.project_cows.id}),
        ...
        with self.get_project_sharing_form_view(self.task_cow.with_context(
                {'tracking_disable': True, 'default_project_id': self.project_cows.id, 'uid': self.user_portal.id}),
                                                self.user_portal) as form:
            form.name = 'Test'
            task = form.save()
            self.assertEqual(task.name, 'Test')
        ...
        task2 = self.env['project.task'] \
            ...
            .create({'name': 'Test'})
        self.assertFalse(task2.portal_user_names,
    ...
    def test_portal_user_cannot_see_all_assignees(self):
        ...
        self.task_cow.message_subscribe(partner_ids=self.user_portal.partner_id.ids)
        self.assertFalse(self.task_cow.with_user(self.user_portal).user_ids,
        ...
        self.assertEqual(self.task_cow.portal_user_names, task_portal_read[0]['portal_user_names'],
    ...
    def test_orm_method_with_true_false_domain(self):
        ...
        self.assertEqual(task, task.filtered_domain(expression.TRUE_DOMAIN),
                         'The task found should be kept since the domain is truly')
        self.assertFalse(task.filtered_domain(expression.FALSE_DOMAIN),
        ...
        self.assertEqual(task_read_group[0]['__count'], 1,
                         'The task should be found with the read_group method containing a truly tuple.')
        self.assertEqual(task_read_group[0]['id'], self.task_portal.id,
                         'The task should be found with the read_group method containing a truly tuple.')
        ...
        self.assertFalse(task_read_group[0]['__count'],
    ...
    def test_add_followers_from_share_edit_wizard(self):
        ...
        task_with_partner_1, task_with_partner_2, task_with_parent_partner, task_without_partner = self.env[
            'project.task'].create([{
        ...
        self.assertEqual(partners, project_to_share.message_partner_ids,
                         "All the partner should be set as a new follower of the project")
        self.assertEqual(partner_a, task_with_partner_1.message_partner_ids,
                         "Only the first partner should be set as a new follower for the task 1")
        self.assertEqual(partner_b, task_with_partner_2.message_partner_ids,
        ...
        self.assertFalse(task_without_partner.message_partner_ids,
    ...
    def test_project_manager_remains_follower_after_sharing(self):
        ...
        self.assertIn(self.user_projectmanager.partner_id, project.message_partner_ids,
