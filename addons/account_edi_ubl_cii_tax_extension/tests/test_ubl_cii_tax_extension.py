...
class TestAccountEdiUblCiiTaxExtension(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.reverse_charge_tax = cls.company_data['default_tax_sale'].copy(
            {'name': 'Reverse charge tax', 'ubl_cii_tax_category_code': 'AE',
             'ubl_cii_tax_exemption_reason_code': 'VATEX_EU_AE'})
        ...
        cls.free_export_tax = cls.company_data['default_tax_sale'].copy(
            {'name': 'Free export tax', 'ubl_cii_tax_category_code': 'G',
    ...
    def test_tax_subtotal(self):
        ...
        for taxes in tax_list:
            invoice = self.env["account.move"].create({
                ...
                "invoice_line_ids": [
                    Command.create({"name": "Test product", "price_unit": 100, "tax_ids": [Command.set(taxes.ids)]})],
            })
            invoice.action_post()
