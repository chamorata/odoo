...
class Message(models.Model):
    ...
    * snailmail.letter (snailmail addon)
    ...
    author_avatar = fields.Binary("Author's avatar", related='author_id.avatar_128', depends=['author_id'],
                                  readonly=False)
    ...
    def _search_has_error(self, operator, operand):
        ...
        return ['!', ('notification_ids.notification_status', 'in', ('bounce',
                                                                     'exception'))]  # this wont work and will be equivalent to "not in" beacause of orm restrictions. Dont use "has_error = False"
    ...
    def _get_forbidden_access(self, operation: str) -> api.Self:
        ...
        for model, docid_msgids in model_docid_msgids.items():
            ...
            if hasattr(documents, '_get_mail_message_access'):
                doc_operation = documents._get_mail_message_access(docid_msgids,
                                                                   operation)  # why not giving model here?
            else:
                doc_operation = self.env['mail.thread']._get_mail_message_access(docid_msgids, operation,
                                                                                 model_name=model)
    ...
    def _notify_message_notification_update(self):
        ...
        for message in messages:
            ...
            if message.author_id and not any(
                    user._is_public() for user in message.author_id.with_context(active_test=False).user_ids):
