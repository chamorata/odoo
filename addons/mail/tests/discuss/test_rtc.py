...
class TestChannelRTC(MailCommon):
    ...
    def test_01_join_call(self):
        ...
        self.maxDiff = None
        channel = self.env['discuss.channel'].channel_create(name='Test Channel',
    ...
    def test_11_start_call_in_group_should_invite_all_members_to_call(self):
        ...
        channel = self.env['discuss.channel'].create_group(
            partners_to=(self.user_employee.partner_id + test_user.partner_id).ids)
    ...
    def test_30_add_members_while_in_call_should_invite_new_members_to_call(self):
        ...
        with self.mock_bus():
            with patch.object(fields.Datetime, 'now', lambda: now + relativedelta(seconds=10)):
                channel.add_members(partner_ids=test_user.partner_id.ids, guest_ids=test_guest.ids,
    ...
    def test_60_rtc_sync_sessions_should_gc_and_return_outdated_and_active_sessions(self):
        ...
        test_session = self.env['discuss.channel.rtc.session'].sudo().create(
            {'channel_member_id': test_channel_member.id})
