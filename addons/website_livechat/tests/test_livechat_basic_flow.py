...
class TestLivechatBasicFlowHttpCase(HttpCaseWithUserDemo, TestLivechatCommon):
    ...
    def test_livechat_username(self):
        ...
        self.assertEqual(channel_1.name,
                         "%s %s" % (f'Visitor #{channel_1.livechat_visitor_id.id}', self.operator.livechat_username))
        channel_1.unlink()
    ...
    def _common_basic_flow(self):
        ...
        self.assertEqual(channel.name,
                         "%s %s" % (f'Visitor #{channel.livechat_visitor_id.id}', self.operator.livechat_username))
        ...
        self.assertEqual(channel.message_ids[0].email_from, self.visitor.display_name,
                         "The sender's email should be the visitor's email.")
        self.assertEqual(channel.message_ids[0].body, "<p>Message from Visitor</p>")
        self.assertEqual(channel.livechat_active, True,
        ...
        self.assertEqual(channel.message_ids[0].author_id, self.operator.partner_id,
                         "The author of the message should be the operator.")
        self.assertEqual(channel.message_ids[0].email_from, self.operator.email,
