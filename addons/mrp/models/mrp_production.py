...
class MrpProduction(models.Model):
    ...
    components_availability_state = fields.Selection([
        ...
        ('expected', 'Expected'),
        ('late', 'Late'),
        ('unavailable', 'Not Available')], compute='_compute_components_availability',
    ...
    show_produce = fields.Boolean(compute='_compute_show_produce',
                                  help='Technical field to check if produce button can be shown')
    show_produce_all = fields.Boolean(compute='_compute_show_produce',
    ...
    @api.depends('state', 'reservation_state', 'date_start', 'move_raw_ids', 'move_raw_ids.forecast_availability',
    ...
    def _compute_delay_alert_date(self):
        delay_alert_date_data = self.env['stock.move']._read_group(
            [('id', 'in', self.move_raw_ids.ids), ('delay_alert_date', '!=', False)], ['raw_material_production_id'],
            ['delay_alert_date:max'])
        delay_alert_date_data = {raw_material_production.id: delay_alert_date_max for
                                 raw_material_production, delay_alert_date_max in delay_alert_date_data}
        for production in self:
    ...
    def _compute_workorder_ids(self):
        for production in self:
            ...
            relevant_boms = [exploded_boms[0] for exploded_boms in production.bom_id.explode(production.product_id, 1.0,
            ...
            deleted_workorders_ids = production.workorder_ids.filtered(
                lambda wo: wo.operation_id and wo.operation_id.bom_id not in relevant_boms).ids
            ...
            if production.product_id != production._origin.product_id or (
                    production._origin.bom_id != production.bom_id and production._origin.bom_id.operation_ids and not production.workorder_ids.filtered(
            ...
            if production.bom_id and production.product_id and production.product_qty > 0:
                ...
                exploded_boms, _dummy = production.bom_id.explode(production.product_id,
                ...
                for bom, bom_data in exploded_boms:
                    ...
                    for operation in bom.operation_ids:
                        if operation.with_context(
                                never_attribute_ids=production.never_product_template_attribute_value_ids)._skip_operation_line(
                                bom_data['product']):
                            workorder = production.workorder_ids.filtered(
                                lambda wo: wo.operation_id == operation and wo.operation_id.bom_id == bom)
                            if workorder:
                ...
                production.workorder_ids = [Command.delete(wo.id) for wo in
                                            production.workorder_ids.filtered(lambda wo: wo.ids and wo.operation_id)]
    ...
    def _compute_reservation_state(self):
        for production in self:
            if production.state in ('draft', 'done', 'cancel'):
                ...
                continue
            relevant_move_state = production.move_raw_ids.filtered(lambda m: not (
                        m.picked or float_is_zero(m.product_uom_qty,
                                                  precision_rounding=m.product_uom.rounding)))._get_relevant_state_among_moves()
    ...
    def _compute_forecasted_issue(self):
        for order in self:
            ...
            if order.product_id:
                virtual_available = order.product_id.with_context(warehouse_id=warehouse.id,
                                                                  to_date=order.date_start).virtual_available
    ...
    @api.depends('product_id', 'bom_id', 'product_qty', 'product_uom_id', 'location_dest_id', 'date_finished',
                 'move_dest_ids', 'never_product_template_attribute_value_ids')
    def _compute_move_finished_ids(self):
        ...
        production_with_move_finished_ids_to_unlink.move_finished_ids = [Command.delete(m) for m in
                                                                         production_with_move_finished_ids_to_unlink.move_finished_ids.ids]
        ...
        for production in production_with_move_finished_ids_to_unlink:
            if production.product_id:
                production.with_context(
                    never_attribute_ids=production.never_product_template_attribute_value_ids)._create_update_move_finished()
            else:
                production.move_finished_ids = [
            ...
            record.is_delayed = bool(
                record.state in ['confirmed', 'progress', 'to_close'] and (
                        record.date_deadline and (
                            record.date_deadline < datetime.datetime.now() or record.date_deadline < record.date_finished))
            )
    ...
    def _onchange_producing(self):
        if self.state in ['draft', 'cancel'] or (self.state == 'done' and self.is_locked):
            return
        productions_bypass_qty_producting = self.filtered(lambda
                                                              p: p.lot_producing_id and p.product_tracking == 'lot' and p._origin and p._origin.qty_producing == p.qty_producing)
    ...
    def copy_data(self, default=None):
        ...
        for production, vals in zip(self, vals_list):
            ...
            if not default or 'move_raw_ids' not in default:
                vals['move_raw_ids'] = [(0, 0, move_vals) for move_vals in
                                        production.move_raw_ids.filtered(lambda m: m.product_qty != 0.0).copy_data()]
        return vals_list
    ...
    def _get_moves_finished_values(self):
        ...
        for production in self:
            ...
            finished_move_values = production._get_move_finished_values(production.product_id.id,
    ...
    def _get_moves_raw_values(self):
        ...
        for production in self:
            ...
            _boms, lines = production.bom_id.explode(production.product_id, factor,
                                                     picking_type=production.bom_id.picking_type_id,
                                                     never_attribute_values=production.never_product_template_attribute_value_ids)
    ...
    def _set_qty_producing(self, pick_manual_consumption_moves=True):
        ...
        for move in (
            ...
            lambda m: m.product_id != self.product_id or m.product_id.tracking == 'serial')
        ):
            ...
            if (not move.manual_consumption or pick_manual_consumption_moves) \
                    and move.quantity \
                    and (
                    move.product_id != self.product_id or not move.production_id or move.product_id.tracking != 'serial'):
                move.picked = True
        ...
        moves_in_first_operation = moves_in_first_operation.filtered(
            lambda move: move.bom_line_id and
                         not move.bom_line_id._skip_bom_line(self.product_id,
                                                             self.never_product_template_attribute_value_ids)
        )
    ...
    def button_unplan(self):
        if any(wo.state == 'done' for wo in self.workorder_ids):
            ...
            raise UserError(
                _("Some work orders have already started, so you cannot unplan this manufacturing order.\n\n"
                  "It’d be a shame to waste all that progress, right?"))
    ...
    def _get_consumption_issues(self):
        ...
        for order in self:
            ...
            for move in order.move_raw_ids:
                ...
                if move.product_id not in expected_qty_by_product and move.picked and not float_is_zero(quantity,
    ...
    def _post_inventory(self, cancel_backorder=False):
        ...
        for order in self:
            ...
            for move in finish_moves:
                move.quantity = float_round(order.qty_producing - order.qty_produced,
                                            precision_rounding=order.product_uom_id.rounding, rounding_method='HALF-UP')
    ...
    def _get_name_backorder(self, name, sequence):
        ...
        if regex.search(name) and (
                max(self.procurement_group_id.mrp_production_ids.mapped("backorder_sequence")) > 1 or sequence > 1):
    ...
    def _split_productions(self, amounts=False, cancel_remaining_qty=False, set_consumed_qty=False):
        ...
        (self.move_raw_ids | self.move_finished_ids).filtered(
            lambda m: m.picked and not m.additional).move_line_ids.filtered(lambda ml: not ml.picked).unlink()
        for production in self:
            for move in production.move_raw_ids | production.move_finished_ids:
                ...
                move.with_context(do_not_unreserve=True,
                                  no_procurement=True).product_uom_qty = production.product_qty * unit_factor
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            if set_consumed_qty and (initial_move.raw_material_production_id or (
        ...
        if not backorders:
            if self.env.context.get('from_workorder'):
                ...
                lines = mos_to_show.move_finished_ids.filtered(
                    lambda m: m.product_id.is_storable and m.state != 'cancel' and m.picked and not m.move_dest_ids)
                if lines:
    ...
    def pre_button_mark_done(self):
        ...
        if quantity_issues:
            ...
            if mos_ask:
                ...
                return self.with_context(always_backorder_mo_ids=mo_ids_always)._action_generate_backorder_wizard(
                    mos_ask)
            elif mo_ids_always:
    ...
    def _auto_production_checks(self):
        ...
        return all(p.tracking == 'none' for p in self.move_raw_ids.product_id | self.move_finished_ids.product_id) \
            or self.product_uom_qty == 1 or (
                        self.product_id.tracking != 'serial' and self.reservation_state in ('assigned', 'confirmed',
                                                                                            'waiting'))
    ...
    def action_merge(self):
        ...
        production.move_raw_ids.move_orig_ids.with_context(
            date_deadline_propagate_ids=set(production.move_raw_ids.ids)).write(
    ...
    def action_plan_with_components_availability(self):
        for production in self.filtered(lambda p: p.state in ('draft', 'confirmed')):
            if production.state == 'draft':
                production.action_confirm()
            move_expected_date = production.move_raw_ids.filtered('forecast_expected_date').mapped(
    ...
    def _link_bom(self, bom):
        ...
        bom_byproducts_by_id = {byproduct.id: byproduct for byproduct in
                                bom.byproduct_ids.filtered(filter_by_attributes)}
        ...
        for move in self.move_raw_ids:
            ...
            for move_line in move.move_line_ids:
                if not move_line.picked or float_is_zero(move_line.quantity,
                                                         precision_rounding=move_line.product_uom_id.rounding) or \
    ...
    def _set_quantities(self):
        ...
        if self.product_tracking == 'serial' and float_compare(self.qty_producing, 1,
                                                               precision_rounding=self.product_uom_id.rounding) == 1:
        ...
        for move in self.move_raw_ids:
            ...
            if move.manual_consumption:
                if move.has_tracking in ('serial', 'lot') and (not move.picked or any(
                        not line.lot_id for line in move.move_line_ids if line.quantity and line.picked)):
    ...
    def _get_autoprint_done_report_actions(self):
        ...
        if productions_to_print:
            action = self.env.ref("mrp.action_report_production_order").report_action(productions_to_print.ids,
                                                                                      config=False)
        ...
        for print_format in productions_to_print.picking_type_id.mapped('mrp_product_label_to_print'):
            ...
            if print_format == 'pdf':
                action = self.env.ref("mrp.action_report_finished_product").report_action(labels_to_print.ids,
                                                                                          config=False)
                clean_action(action, self.env)
                report_actions.append(action)
        ...
        if self.env.user.has_group('mrp.group_mrp_reception_report'):
            ...
            if reception_reports_to_print:
                action = self.env.ref('stock.stock_reception_report_action').report_action(reception_reports_to_print,
                                                                                           config=False)
                action['context'] = dict({'default_production_ids': reception_reports_to_print.ids}, **self.env.context)
                clean_action(action, self.env)
                report_actions.append(action)
            reception_labels_to_print = self.filtered(lambda
                                                          p: p.picking_type_id.auto_print_mrp_reception_report_labels and p.picking_type_id.code == 'mrp_operation')
            if reception_labels_to_print:
                ...
                if moves_to_print:
                    ...
                    quantities = ','.join(
                        str(qty) for qty in moves_to_print.mapped(lambda m: math.ceil(m.product_uom_qty)))
                    data = {
        ...
        if self.env.user.has_group('stock.group_production_lot'):
            productions_to_print = self.filtered(
                lambda p: p.picking_type_id.auto_print_done_mrp_lot and p.move_finished_ids.move_line_ids.lot_id)
            productions_by_print_formats = productions_to_print.grouped(
            ...
            for print_format in productions_to_print.picking_type_id.mapped('done_mrp_lot_label_to_print'):
                ...
                if print_format == 'pdf':
                    action = self.env.ref("stock.action_report_lot_label").report_action(lots_to_print.ids,
                                                                                         config=False)
                    clean_action(action, self.env)
                    report_actions.append(action)
                elif print_format == 'zpl':
