    ...
    def _get_demo_data(self, company=False):
        ...
        if company.chart_template.startswith('be'):
            ...
            account_data.update({
                ...
                f"account.{cid}_a7600": {
                    'tag_ids': [Command.link(self.env.ref('account.demo_sale_of_land_account').id)]},
                f"account.{cid}_a6201": {'tag_ids': [Command.link(self.env.ref('account.demo_ceo_wages_account').id)]},
                f"account.{cid}_a240000": {
                    'tag_ids': [Command.link(self.env.ref('account.demo_office_furniture_account').id)]},
            })
